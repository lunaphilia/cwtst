...
# CloudWatchに登録する値
usage=$(curl -s http://127.0.0.1:24220/api/plugins.json | jq . | grep retry | cut -d ":" -f 2 | grep -o -E [0-9]+ | awk 'BEGIN {n=0} {n=n+$1} END {print n}')
...
cw_opts=${cw_opts}" --metric-name TDAgentRetry"
